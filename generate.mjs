        ...
        <div class="sheet-body" style="text-align:center">
          ...
          <div class="tts-control" style="text-align:left">
            ...
            <div class="tts-toggle-group" style="display:flex;gap:8px;margin:8px 0 16px">
              <button class="tts-toggle-btn active" id="tts-lang-zh" onclick="setTTSLang('zh-CN')" aria-label="普通話">普通話</button>
              <button class="tts-toggle-btn" id="tts-lang-cantonese" onclick="setTTSLang('zh-HK')" aria-label="廣東話">廣東話</button>
            </div>
            <div class="tts-label"><span>音色</span></div>
            <div class="tts-toggle-group" style="display:flex;gap:8px;margin:8px 0 16px">
              <button class="tts-toggle-btn active" id="tts-voice-male" onclick="setTTSVoice('male')" aria-label="男聲">👨 男聲</button>
              <button class="tts-toggle-btn" id="tts-voice-female" onclick="setTTSVoice('female')" aria-label="女聲">👩 女聲</button>
            </div>
...
window.setTTSVoice=function(v){
        ...
        if(maleBtn)maleBtn.classList.toggle('active',v==='male');
        if(femaleBtn)femaleBtn.classList.toggle('active',v==='female');
        try{localStorage.setItem('dn_tts_voice',v)}catch(e){}
      };
      
      window.setTTSLang=function(lang){
        ttsCurrentLang=lang;
        var zhBtn=document.getElementById('tts-lang-zh');
        var cantoneseBtn=document.getElementById('tts-lang-cantonese');
        if(zhBtn)zhBtn.classList.toggle('active',lang==='zh-CN');
        if(cantoneseBtn)cantoneseBtn.classList.toggle('active',lang==='zh-HK');
      ...
      (function(){
        try{
          ...
          if(savedVoice){ttsCurrentVoice=savedVoice;setTTSVoice(savedVoice)}
          var savedLang=localStorage.getItem('dn_tts_lang');
          if(savedLang){ttsCurrentLang=savedLang;setTTSLang(savedLang)}
        }catch(e){}
      })();
