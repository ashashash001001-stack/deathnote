...
function coverMini(book, w, h, showStatus = false) {
  ...
  const seed = bookCoverSeed(book);
  const decorations = coverDecorations(seed);
  const statusBadge = showStatus ? `<span class="status-badge" style="background:${book.status==='completed'?'#A3B18A':'#7DB8F0'}">${book.status==='completed'?'完結':'連載'}</span>` : '';
  return `<div class="css-book-cover css-book-cover-mini" style="--hue:${hue};--hue2:${hue2};width:${width}px;height:${height}px;background:linear-gradient(135deg,hsl(${hue},75%,60%),hsl(${hue2},65%,35%))">
    <div class="cover-decorations" style="background-image:${decorations};background-repeat:no-repeat"></div>
    <div class="cover-content"><h2 class="cover-title" style="font-size:14px">${book.title.slice(0,2)}</h2>${statusBadge}</div>
  </div>`;
}
