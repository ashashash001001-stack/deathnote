...
BOOKS.forEach(book => {
  ...
  bookChapters.forEach((ch, idx) => {
    ...
    const body = `<div class="reader-page"><main class="reader-immersive" id="reader-el">
      ...
      <header class="reader-topbar" id="reader-topbar" role="banner">
        <div class="reader-topbar-inner">
          <a href="${SITE.base}/book/${book.id}" class="btn-press" style="display:flex;align-items:center;gap:8px" aria-label="返回 ${book.title} 詳情">
      ...
      <footer class="reader-bottombar" id="reader-bottombar" role="contentinfo"><div class="reader-bottombar-inner">
        ${prev ? `<a href="${SITE.base}/book/${book.id}/${prev.id}" class="nav-btn btn-press" aria-label="上一章：${prev.title}"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>上一章</a>` : '<span class="nav-btn disabled" aria-label="已是第一章"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>上一章</span>'}
        ...
        ${next ? `<a href="${SITE.base}/book/${book.id}/${next.id}" class="nav-btn btn-press" aria-label="下一章：${next.title}">下一章<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg></a>` : '<span class="nav-btn disabled" aria-label="已是最後一章">下一章<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg></span>'}
      </div></footer>
    ...
    function setTTSRate(v){document.getElementById('tts-rate-val').textContent=parseFloat(v).toFixed(1)+'x';if(ttsUtterance){ttsUtterance.rate=parseFloat(v)}}
    function setTTSPitch(v){document.getElementById('tts-pitch-val').textContent=parseFloat(v).toFixed(1);if(ttsUtterance){ttsUtterance.pitch=parseFloat(v)}}
    function seekTTSRange(val){var pct=parseInt(val)/100;var paras=document.querySelectorAll('#reader-content p[data-tts-idx]');var total=paras.length;if(!total)return;var targetIdx=Math.min(Math.floor(pct*total),total-1);if(targetIdx<0)targetIdx=0;if(ttsPlaying){speakFrom(targetIdx)}else{ttsCurrentIdx=targetIdx;updateTTSProgress(targetIdx/(total-1));highlightParagraph(targetIdx)}}
    function speakFrom(idx){speechSynthesis.cancel();var paras=document.querySelectorAll('#reader-content p[data-tts-idx]');if(idx<0||idx>=paras.length){ttsPlaying=false;updateTTSIcon();updateTTSProgress(0);clearTTSHighlight();return}ttsCurrentIdx=idx;ttsCharOffset=0;var txt='';for(var i=idx;i<paras.length;i++){txt+=paras[i].textContent+' '}ttsUtterance=new SpeechSynthesisUtterance(txt);ttsUtterance.lang='zh-TW';var r=document.getElementById('tts-rate');var p=document.getElementById('tts-pitch');if(r)ttsUtterance.rate=parseFloat(r.value);if(p)ttsUtterance.pitch=parseFloat(p.value);ttsUtterance.onend=function(){ttsPlaying=false;ttsCurrentIdx=0;ttsCharOffset=0;updateTTSIcon();updateTTSProgress(0);clearTTSHighlight();var rng=document.getElementById('tts-range');if(rng)rng.value=0};ttsUtterance.onboundary=function(ev){if(ev.name==='word'||ev.name==='sentence'){var charPos=ev.charIndex;var before=0;var done=idx;for(var j=idx;j<paras.length;j++){var plen=paras[j].textContent.length;if(charPos<before+plen+1){done=j;ttsCharOffset=charPos-before;break}before+=plen+1}if(done!==ttsCurrentIdx){ttsCurrentIdx=done;highlightParagraph(done)}var pct=(done-idx)/(paras.length-idx);updateTTSProgress(pct);var rng=document.getElementById('tts-range');if(rng)rng.value=Math.round(pct*100)}}};speechSynthesis.speak(ttsUtterance);ttsPlaying=true;updateTTSIcon();updateTTSProgress(0);highlightParagraph(idx);var rng=document.getElementById('tts-range');if(rng)rng.value=0}
